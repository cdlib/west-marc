...
# $Revision: 1.10 $
# $Author: smcgovrn $
# $Date: 2002/12/16 22:00:29 $
# $Id: MARC.pm,v 1.10 2002/12/16 22:00:29 smcgovrn Exp $
#
#-------------------------------------------------------------------------------
package MARC;
...
use Carp;
use strict;
use vars qw($CDLVERSION $VERSION @ISA @EXPORT @EXPORT_OK %EXPORT_TAGS $DEBUG $TEST);

$MARC::CDLVERSION = '$Revision: 1.10 $';
...
sub new {                               # rec
    ...
    if ($file) {
        unless (-e $file) {
            mycarp "File $file doesn't exist"; return;
        }
    ...
    print STDERR "read in $totalrecord records\n" if $MARC::DEBUG;
    return $marc;
}
####################################################################
...
sub openmarc {
	...
	#$$debug$$
	#print STDERR "MARC::openmarc()\n";
	#print STDERR "MARC::openmarc() - file = '$file'\n";

    if ($file ne "&STDIN" && not(-e $file)) {
        mycarp "File \"$file\" doesn't exist"; return;
    }
	...
	# skip over first n records if specified
	my $skip = $params->{'skip'} || 0;
	$skip =~ s/\s//g;
	#$$debug$$
	#print STDERR "MARC::openmarc() - params->skip = '", $params->{'skip'}, "'\n";
	#print STDERR "MARC::openmarc() - skip = '$skip'\n";
	if ($skip =~ m/^\d+$/)
	{
		local $/ = "\035";              # set EOL to 0x1D
		...
		while ($readcnt < $skip)
		{
			my $line = <file>;
			$readcnt++;
			#print STDERR "skipping #$readcnt: $line\n";
		}
	}
...
#
# If increment is negative the current marc object will be cleared
# prior to reading.
####################################################################
sub nextmarc {
    my $marc=shift;
	...
	{
		#$$debug$$
		#print STDOUT "nextmarc() - Deleting before reading - increment = '$increment'\n";
		#print STDOUT ("nextmarc() - before delete number of marc recs in object = '",
		#			  (scalar(@$marc)), "'\n");
		# reverse the sign so _readmarc can use $increment
		$increment *= -1;

		#delete all records
		my $class = ref $marc;
		my @newmarc = @$marc[0];        # array slice, look it up.
		@$marc = @newmarc;
		bless $marc,$class;

		#$$debug$$
		#print STDOUT "nextmarc() - after delete - increment = '$increment'\n";
		#print STDOUT ("nextmarc() - after delete number of marc recs in object = '",
		#			  (scalar(@$marc)), "'\n");
	}

    if ($increment) {
        $marc->[0]{'increment'}=$increment;
    }
    if ($marc->[0]{'format'} =~ /usmarc/oi) {
        $totalrecord = _readmarc($marc);
    }
...
sub deletemarc {
	...
	#$$debug$$
	#print STDERR "deletemarc() keepers = '", (join(':',@keepers)), "'\n";
    ...
    for (my $i=1; $i<=$#$marc; $i++) {
        ...
        if ($searchtype eq "fieldpresence") {
            ...
            foreach my $y (@$x) {
                ...
                print STDERR "DEBUG: searching field '$z' for '$regex'\n" if $MARC::DEBUG;
                if (eval qq("$z" =~ $regex)) {
                    $flag=1;
                    last;
                }
            ...
            foreach my $y (@$x) {
                print STDERR "DEBUG: searching subfield '$$y' for '$regex'\n" if $MARC::DEBUG;
                if (eval qq("$$y" =~ $regex)) {
                    $flag=1;
                    last;
                }
...
sub getfirstvalue {                     # rec
    ...
    if (not($record)) {
        mycarp "You must specify a record"; return;
    }
    if ($record > $#{$marc}) {
        mycarp "Invalid record specified"; return;
    }
...
sub addfield {
    ...
    my $field = $params->{'field'};
    unless ($field) {
        mycarp "You must specify a field"; return;
    }
    unless ($field =~ /^\d{3}$/) {
        mycarp "Invalid field specified"; return;
    }
    ...
    unless (defined $value[0]) {
        mycarp "No value specified"; return;
    }
...
sub deletefirst {                       # rec
    ...
    if (!$recnum) {
        mycarp "Need a record to confine my destructive tendencies"; return undef;
    }
...
sub updatefirst {                       # rec
    ...
    my $recnum = $template->{'record'};
    if (!$recnum) {
        mycarp "Need a record to confine my changing needs."; return undef;
    }
...
sub _offset{
    my $value=shift;
    my $digits=shift;
    print STDERR "DEBUG: _offset value = $value, digits = $digits\n" if $MARC::DEBUG;
    my $x=length($value);
    $x=$digits-$x;
    $x="0"x$x."$value";
}
...
sub _writemarc {                        # rec
    ...
    foreach my $field (@{$record->{'array'}}) {
        ...
        if ($tag < 10) {
            ...
            if ($MARC::DEBUG)
            {
                my $max = scalar @subfields;
                print STDERR "DEBUG: field $tag has $max subfields\n";
                for (my $i = 0; $i < $max; $i++)
                {
                    print STDERR "DEBUG: subfield[$i] = '$subfields[$i]'\n";
                    if ($subfields[$i] =~ /\036$/)
                    {
                        print STDERR "DEBUG: subfield[$i] ends in field delimter\n";
                    }
                }
            } # end debug
            while (@subfields) {
...
sub _marc2ascii {
    ...
    foreach my $fields (@{$record->{'array'}}) { #cycle each field
        my $tag=$fields->[0];
        $tag = "LDR" if $tag eq "000";  # CDL change for RAD by SMG
        print STDERR "ASCII: tag = $tag\n" if $MARC::DEBUG;
        if ($tag<10) {
            $output.="$tag  $fields->[1]";
        }
        else {
...
sub _char2maker {
    ...
    my $charmap = shift;
    # modernized hash dereference for cdl - smg 2002/08/16
    my $maker_string = join ('', map { $charmap->{$_} } @marc_string);
    print STDERR "DEBUG _char2maker: maker_string = $maker_string\n";
...
sub ustext_default {
    ...
    if ($MARC::DEBUG) {
        print STDERR "DEBUG ustext_default: printing charmap\n";
        foreach my $num (sort keys %outchar) {
            printf STDERR "DEBUG ustext_default: %x = %s\n", ord($num), $outchar{$num};
        }
    }
    return \%outchar;
}
...
sub _readmarc {                         # rec
    ...
    my $line;
	#$$debug$$
	#print STDERR "MARC::Rec::_readmarc()\n";

    $line = $string if $string;
    $line = <$handle> if $handle and !defined($string);
    my $recordlength = substr($line,0,5);
    my $octets = length ($line);
    # The following blind removal of 0x1A (\cZ) causes the fields containing
    # the character to now contain the field delimiter as part of the data.
    # Bad! Bad! Bad!
    # In fact if we remove embedded \n and \r that too will cause all kinds
    # of mayhem. These need to be done at the subfield data element level,
    # after parsing, if at all.
    # Removed by smg. 09/20/2002
    #$line=~s/[\n\r\cZ]//og;
    return (undef,undef) unless $line;
    if ($recordlength =~ /\d{5}/o) {
        print STDERR "recordlength = $recordlength, length = $octets\n"	if $MARC::DEBUG;
...
sub _maker2char {                       # rec
    ...
    while ($marc_string =~ /{(\w{1,8}?)}/o) {
        # modernized hash dereference for cdl - smg 2002/08/16
        print STDERR "DEBUG _maker2char: parsed '$1'\n" if $MARC::DEBUG;
        if (exists $charmap->{$1}) {
            print STDERR "DEBUG _maker2char: mapping '$1' to '$charmap->{$1}'\n" if $MARC::DEBUG;
...
sub usmarc_default {                    # rec
    ...
    if ($MARC::DEBUG) {
        print STDERR "DEBUG usmarc_default: printing charmap\n";
        foreach my $str (sort keys %inchar) {
            printf STDERR "DEBUG usmarc_default: %s = %x\n", $str, ord($inchar{$str});
        }
    }
    return \%inchar;
}
...
sub updatefirst {                       # rec
    ...
    if (!$field) {
        mycarp "Need a field to configure my changing needs."; return undef;
    }
    ...
    if ($field and not($subfield)) {
        ...
        $ufield[2]=$i2;
        if ($field <10) {
            @ufield = ($field,$ufield_lt_10_value);
        }
    ...
    if ($field and $subfield) {
        if ($field <10) {
            mycarp "Cannot update subfields of control fields"; return undef;
        }
...
sub getmatch {                          # rec
    ...
    my $tag = $rfield->[0];
    if ($tag < 10) {
        mycarp "can't find subfields or indicators for control fields"; return undef;
    }
...
sub deletesubfield {                    # rec
    ...
    my $tag = $rfield->[0];
    if ($tag < 10) {
        mycarp "Can't use subfields or indicators for control fields"; return undef;
    }

    if ($subf =~/i[12]/) {
        mycarp "Can't delete an indicator."; return undef;
    }
    my $i=3;
...
sub insertpos {                         # rec
    ...
    if ($subf =~/i[12]/) {
        mycarp "Can't insert past an indicator."; return undef;
    }
    my $i=3;
    for ($i=3;$i<@$rfield;$i+=2) {
...
sub getfirstvalue {                     # rec
    ...
    if ($field and not $subfield) {
        ...
        if ($field <10) {
            mycarp "There are no subfields or indicators for control fields";return;
        }
