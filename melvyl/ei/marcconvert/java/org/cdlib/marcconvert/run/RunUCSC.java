...
import org.cdlib.marcconvert.UCSCConvert;
import org.cdlib.marcconvert.RunConvert;
import org.cdlib.util.marc.MarcParmException;


/**
 * This class performs conversion of marc records from the UCSC source
 * by passing a UCSCConvert converter object to RunConvert and invoking
 * the RunConvert.convert() method.
 *
 * @author <a href="mailto:rmoon@library.berkeley.edu">Ralph Moon</a>
 * @author <a href="mailto:shawnm@splorkin.com">Shawn McGovern</a>
 * @version $Id: RunUCSC.java,v 1.6 2002/11/12 21:41:10 smcgovrn Exp $
 */
public class RunUCSC
{
	/**
	 * log4j Logger for this class.
	 */
    private static Logger log = Logger.getLogger(RunUCSC.class);

	/**
	 * CVS header string.
	 */
    public static final String cvsHeader = "$Header: /cvs/root/melvyl/ei/marcconvert/java/org/cdlib/marcconvert/run/RunUCSC.java,v 1.6 2002/11/12 21:41:10 smcgovrn Exp $";
    ...
    public static void main(String args[])
    {
        try
        {
            ...
            t.setName("RunUCSC_" + startDate.getTime());

            log.info("UCSC conversion started");

            UCSCConvert ucsc = new UCSCConvert();

            // Get a new RunConvert using our conversion class.
            RunConvert run = new RunConvert(ucsc);

            // RunConvert.convert() handles all I/O and the calling
            // of the MarcConvert routines to perform the conversion.
            int rc = run.convert();
            log.info("UCSC conversion completed - return code = " + rc);
