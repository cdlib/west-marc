...
import org.cdlib.marcconvert.UCMConvert;
import org.cdlib.marcconvert.RunConvert;
import org.cdlib.util.marc.MarcParmException;


/**
 * This class performs conversion of marc records from the UCM (Merced) source
 * by passing a UCMConvert converter object to RunConvert and invoking
 ...
 * @version $Id: RunUCM.java,v 1.1 2005/04/27 16:34:06 rkl Exp $
 */
public class RunUCM
{
	/**
	 ...
	 */
    private static Logger log = Logger.getLogger(RunUCM.class);

	/**
	 * CVS header string.
	 */
    public static final String cvsHeader = "$Header: /cvs/root/melvyl/ei/marcconvert/java/org/cdlib/marcconvert/run/RunUCM.java,v 1.1 2005/04/27 16:34:06 rkl Exp $";
    ...
    public static void main(String args[])
    {
        try
        {
            ...
            t.setName("RunUCM_" + startDate.getTime());

            log.info("UCM conversion started");

            UCMConvert mer = new UCMConvert();

            // Get a new RunConvert using our conversion class.
            RunConvert run = new RunConvert(mer);

            // RunConvert.process handles all I/O and the calling
            // of the MarcConvert routines to perform the conversion.
            int rc = run.convert();
            log.info("UCM conversion completed - return code = " + rc);
