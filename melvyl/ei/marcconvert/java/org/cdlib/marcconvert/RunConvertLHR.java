 ...
 * @version $Id: RunConvertLHR.java,v 1.4 2010/04/30 22:33:36 aleph16 Exp $
 */
public class RunConvertLHR
    implements ConvertConstants, MarcConstants
{
	/**
	 * log4j Logger for this class.
	 */
    private static Logger log = Logger.getLogger(RunConvertLHR.class);

	/**
	 * CVS header string.
	 */
    public static final String cvsHeader = "$Header: /cvs/root/melvyl/ei/marcconvert/java/org/cdlib/marcconvert/RunConvertLHR.java,v 1.4 2010/04/30 22:33:36 aleph16 Exp $";
    ...
    private MarcConvertLHR        converter = null;

    // create marc output in Hashtable
    private Hashtable hashOutMarc = new Hashtable();
    // save input configuration for processing LHR records
    private Properties configIn      = null;
    ...
    //====================================================
    /**
     * RunConvertLHR - contructor - set subclass for marc conversion
     * @param inMarcConvert - Marc conversion subclass of MarcConvert
     */
    public RunConvertLHR(MarcConvertLHR inMarcConvert)
    ...
    public int convert()
    {
		...
		checkLFCR(marcRecIn);
	        ...
	        check7f(marcRecIn);
                ...
                hashOutMarc = new Hashtable();
                //import - use convertLHR instead of convert
                convertStatus = converter.convertLHR(marcRecIn, marcRecOut, hashOutMarc, configIn);
	            ...
	            // create a shared print field (793), if needed
                    ...
                    case CONVERT_REC_SUCCESS:
                        Enumeration codes = hashOutMarc.keys();
                        while (codes.hasMoreElements())
                        {
                            String campusCode = (String) codes.nextElement();
                            MarcRecord campusRecMarc = (MarcRecord) hashOutMarc.get(campusCode);
                            String campusRecString = campusRecMarc.marcDump();
 
                            // Catch it if record contains no 856s.
                            MarcFieldList fList856 = campusRecMarc.allFields("856");
                            if (fList856.size() == 0)
                            {
                                // Mark it as ERROR record and write it to ERROR file.
                                writeError(errors, campusRecString);
                                errorCnt = addRunStat(MarcConvert.ERROR);
                                addErrorStat(ConvertMessages.ERR_CODE_NO_856_FIELD);
                            }
                            else
                            {
                                writeMarc(marcOut, campusRecString);
                                cntAll += 1;
                            }
                        }
    ...
    /**
     * close - close all lhr open files to flush buffers
     */
    private void close()
    {
        try
    ...
    private Properties processConfiguration()
    {
        ...
        // save a copy of input configuration for processing LHR records
        configIn = pin; 
    ...
    private void doLabel()
    {
        ...
        String labelDir = configIn.getProperty("labeldir");
        if (labelDir == null)
        {
            log.error("Found no 'labeldir' entry in configuration file" );
            throw new MarcParmException(this, "configuration error: " + "labeldir not found");
        }
        File outFile = new File(configIn.getProperty("outfile"));
        String fn = outFile.getName().toUpperCase();
        String regex = ".*([dD]\\d{4}.*$)";
        ...
        String labelFile = labelDir + "LHR.LABEL." + fn ;
        ConvertFile lb = new ConvertFile("label", labelFile);
        // get the last element of output filename path
        File of = new File(outFileName);
        String ofn = of.getName().toUpperCase();

        // get ORS value from config file
        String ors = configIn.getProperty("ors");
        if (ors == null)
        {
            log.error("Found no 'ors' entry in configuration file" );
            throw new MarcParmException(this, "configuration error: " + "ors not found");
        }

        // get FDI value from config file
        String fdi = configIn.getProperty("fdi");
        if (fdi == null)
        {
            log.error("Found no 'fdi' entry in configuration file" );
            throw new MarcParmException(this, "configuration error: " + "fdi not found");
        }

        lb.write("DAT  " + getAnsiDateTime() + EOL);
        lb.write("RBF  " + cntAll + EOL);
        lb.write("DSN  " + ofn + EOL);
        lb.write("ORS  " + ors.toUpperCase() + EOL);
        lb.write("FDI  " + fdi.toUpperCase() + EOL + EOL);
    ...
    private String constructCampusOutFileName(String fName, String fParent, String c)
    {
        String[] arr = configIn.getProperty(c).split(",");
        String cfn = new String(fParent + "/" + arr[0].toUpperCase() + "." + fName);
        return cfn;
    }
