...
import org.cdlib.marcconvert.ConvertMessages;
import org.cdlib.marcconvert.MarcConvert;
import org.cdlib.marcconvert.MarcDropException;
import org.cdlib.marcconvert.MarcFailException;
import org.cdlib.util.marc.DuplciateBibRecord;
...
/**
 * This class runs the Marc conversion using a subclass of MarcConvert
 * 
 * References:
 * https://diva.cdlib.org/projects/melvyl/ei/ConverterSpecifications/
 * En.STF.Jav.doc
 * https://diva.cdlib.org/projects/melvyl/ei/ConverterSpecifications
 * /sort_separate_records.doc
 * 
...
/*
 * Change history: 06/03/2011 dbb - Added comments re: processing specific to
 * STF
 */

public class CPEMerge implements ConvertConstants, MarcConstants {
	 ...
	 */
	private static Logger log = Logger.getLogger(CPEMerge.class);
	...
	private static TreeMap<String, Integer> runStats = new TreeMap<String, Integer>();
	private static TreeMap<String, Integer> dirStats = new TreeMap<String, Integer>();
	private static TreeMap<String, Integer> errorStats = new TreeMap<String, Integer>();
	private static TreeMap<String, Integer> rejectStats = new TreeMap<String, Integer>();
	 ...
	 * 
	 */
	public CPEMerge() throws MarcParmException {
	}
	...
	/**
	 * Handle the conversion process. This method replaces the obsolete method
	 * process().
	 */
	public int initialize() {
		...
		try {
			...
			marcOut = new OutputStreamWriter(new BufferedOutputStream(
					new FileOutputStream(outFileName), 32 * 1024), "iso-8859-1");

			// Buffer the records converted with errors in a 32k buffer
			errors = new OutputStreamWriter(new BufferedOutputStream(
					new FileOutputStream(errorFileName), 32 * 1024),
					"iso-8859-1");

			// Buffer the rejected records in a 32k buffer
			rejects = new OutputStreamWriter(new BufferedOutputStream(
					new FileOutputStream(rejectFileName), 32 * 1024),
					"iso-8859-1");
	...
	/*
	 * 6/3/2011 dbb Program is invoked by
	 * org.cdlib.marcconvert.run.RunENLAMerge.java ENLAMerge ucla = new
	 * ENLAMerge(); int rc = ucla.doCombine(); Configuration from a properties
	...
	public int merge() {
		...
		for (int i = skip; i < maxIn; i++) {
			...
			if (status.recordStatus == CONVERT_EOF)
				break;
	...
	public int addStat(Status status) {
		...
		try {
			...
			case CONVERT_REC_REJECT:
				...
				if (status.id != null) {
					log.error("Record Rejected: (id=" + status.id + ") R-"
							+ rejectCnt + " " + statusMsg);
				} else {
					log.error("Record Rejected: I-" + inputCnt + " R-"
							+ rejectCnt + " " + statusMsg);
				}
				break;
			...
			log.error(
					"Exception: for record (I-" + inputCnt + "): "
							+ e.getMessage(), e);
	...
	/*
	 * 6/3/2011 dbb Processing MAY be specific to STF.
	 */
	...
	private Status processMergeRecord(int inx, MarcStream marcReader) {
		...
		try {
			...
			if (marcRecIn == null) {
				log.error("Record # " + inputCnt
						+ " input routine returned null MarcRecord");
			...
			if (log.isDebugEnabled()) {
				log.debug("rec # " + inputCnt + " convert status = "
						+ status.recordStatus);
			}
			...
			log.debug("Trapped MarcEndOfFileException - rec count = "
					+ inputCnt);
			...
			log.error(
					"MarcException for record (I-" + inputCnt + "): "
							+ e.getMessage(), e);
			status.recordStatus = CONVERT_REC_FAILURE;
	...
	/**
	 * Get current date and time string compliant with ANSI X3.30 (data) and
	 * ANSI X3.43 (time). This is the format required for Marc tag 005.
	 * 
	 * @return formatted date time string
	 */
	private String getAnsiDateTime() {
		SimpleDateFormat ansiFormatter = new SimpleDateFormat(
				"yyyyMMddHHmmss'.0'");
	...
	private boolean isValidAnsiDateTime(String datetime) {
		...
		if (datetime == null || datetime.length() != 16) {
			...
			SimpleDateFormat ansiFormatter = (SimpleDateFormat) DateFormat
					.getDateTimeInstance();
	 ...
	 * Required entries:
	 * 
	 * infile - The marc data file to convert outfile - The converted marc data
	 * file errorfile - A file to hold those records converted with errors
	 * rejectfile - A file to hold reject records reportfile - The report file
	 * for this run
	 * 
	 ...
	 * overriding the 005 in any input records. If 'N' the filedate will only be
	 * used to create 005 fields for those input records that do not contain a
	 * 005. Default is 'N'.
	 * 
	 * skip - The number of input records to skip before conversion starts.
	 * Default's to 0, if the entry is not specified or is not numeric. Keyword
	 * values 'none' and 'all' may be specified.
	 * 
	 * maxin - The maximum number of records to read from the file. This
	 * includes the number of records to skip, so if skip is greater than or
	 * equal maxin no records will be processed. Default's to 'all', if the
	 * entry is not specified or is not numeric. Keyword values 'none' and 'all'
	 * may be specified.
	 * 
	 * maxout - The maximum number of records to convert. Rejected records are
	 * not included in this count. Default's to 'all', if the entry is not
	 * specified or is not numeric. Keyword values 'none' and 'all' may be
	 * specified.
	 * 
	 * debuglevel - Controls the level of debug information written to the
	 * report file. A higher number will get more debug information. The actual
	 * meaning of this number is varies among conversion programs. The highest
	 * meaningful value is 3, which will ascii print each input and output marc
	 * records. To suppress debugging output specify 0. Default's to 0, if the
	 * entry is not specified or is not numeric. Keyword value 'none' may be
	 * specified.
	 * 
	 * validateout - Perform post-conversion validation when this is 'Y', and
	 * byapss validation of output records when it is 'N'. Default is 'Y'.
	 * 
	 */
	private Properties processConfiguration() {
		...
		if (configfile == null || configfile.length() == 0) {
			throw new MarcParmException(this,
					"-Dconfig=<name> required on java command");
		}

		try {
			pin.load(new FileInputStream(configfile));
		} catch (Exception exception) {
			throw new MarcParmException(this, "configuration error: "
					+ exception);
		}
		...
		if (inFileName.length() == 0) {
			throw new MarcParmException(this,
					"infile - input file name not supplied");
		}
		...
		if (outFileName.length() == 0) {
			throw new MarcParmException(this,
					"outfile - output file name not supplied");
		}
		...
		if (outDirName.length() == 0) {
			throw new MarcParmException(this,
					"outdir - output file name not supplied");
		}
		...
		if (reportFileName.length() == 0) {
			throw new MarcParmException(this,
					"reportfile - report file name not supplied");
		}
		...
		if (errorFileName.length() == 0) {
			throw new MarcParmException(this,
					"errorfile - error file name not supplied");
		}
		...
		if (rejectFileName.length() == 0) {
			throw new MarcParmException(this,
					"rejectfile - reject file name not supplied");
		}
		...
		if ((fileDate = pin.getProperty("filedate")) == null) {
			throw new MarcParmException(this,
					"filedate - filedate not supplied");
		}
		...
		/*
		 * Get the debug level. This affects the report output, not the logging
		 * level (which is set in the log4j properties file.
		 */
		 ...
		 * Get validateOut
		 */
		validateOut = !(pin.getProperty("validateout", "Y")
				.equalsIgnoreCase("N"));
		...
		if (log.isDebugEnabled()) {
			log.debug(" configfile:" + configfile + " infile:" + inFileName
					+ " outfile:" + outDirName + " reportfile:"
					+ reportFileName + " errorfile:" + errorFileName
					+ " rejectfile:" + rejectFileName + " loctabfile:"
					+ loctabFileName + " filedate:" + fileDate + " forcedate:'"
					+ (forceDate ? "Y" : "N") + "'" + " skip:" + skip
					+ " maxin:" + maxIn + " maxout:" + maxOut + " debuglevel:"
					+ debugLevel + " validateout:" + (validateOut ? "Y" : "N")
					+ " benchrun:" + (benchRun ? "Y" : "N") + " benchdate:"
					+ benchRunDateStr);
		}
	...
	private void reportResults() {
		...
		if (rejectStats.size() > 0) {
			...
			for (Iterator<String> iter = (rejectStats.keySet()).iterator(); iter
					.hasNext();) {
		...
		if (errorStats.size() > 0) {
			...
			for (Iterator<String> iter = errorStats.keySet().iterator(); iter.hasNext();) {
				String key = (String) iter.next();
	...
	private void reportRunStat(int value, String text) {
		report.write("    " + F.f(value, 8, F.RJ) + " - " + text + EOL);
	}
	 ...
	 * 
	 * @param marcStr
	 *            - String containing the marc record to output
	 */
	 ...
	 *            - the Writer to use for output
	 * @param buffer
	 *            - StringBuffer containing the marc record to output
	 */
	...
	/*
	 * 6/3/2011 dbb We are working with a single file that contains both
	 * bibliographic records and holdings records, not necessarily in order.
	 * 
	 ...
	 * 
	 * STF bib records: 001 contains the bib record number 004 is not present
	 * STF holdings records: 001 contains some other number (if present) 004
	 * contains the bib record number
	 * 
	 * Intermediate output is to a temporary directory (deleted later).
	 * Directory structure is defined by the id (bib record number). For
	 * example: All bib and holdings records for bib record number: 3806866 Are
	 * in this output file: outDirName/380/686/6/marc.txt The program invokes
	 * writeAppendMarc to append matching records to the file
	 */
	...
	/*
	 * 6/3/2011 dbb To append records (boolean append = true): FileOutputStream
	...
	/*
	 * 6/3/2011 dbb This step invokes processAllFiles to recursively process the
	 * temporary files written by marcToDir.
	 */
	...
	/*
	 * 6/3/2011 dbb This step walks the directory structure and calls
	 * combineFile to process each marc.txt file. Each file at this point
	 * contains all the MARC bib and holdings records for a bib record number
	 * (in no particular order).
	 */

	private void processAllFiles(File dir) throws MarcFailException {
		if (log.isDebugEnabled()) {
	...
	/*
	 * 6/3/2011 dbb This step invokes buildCombinedRecord to build a single MARC
	 * record containing both bib records and holdings.
	 */
	...
	private int combineFile(File dir) {
		...
		try {
			...
			Vector<MarcRecord> arr = getRecords(dir);
			if (arr == null) {
			...
			if (log.isDebugEnabled()) {
				log.debug("rec # " + mergeCnt + " convert status = "
						+ status.recordStatus);
			}
	...
	/*
	 * 6/3/2011 dbb This step includes processing specific to STF in the logic
	 * to identify which of the records is the bib record. For STF records, the
	 * bib record (or base record) does not have an 004. (See marcToDir.)
	 */

	private Status buildCombinedRecord(File dir, Vector<MarcRecord> arr, MarcRecord outMarc) {
		...
		for (int i = 0; i < arr.size(); i++) {
			...
			bibId001 = marcRec.getLeaderValue();
				if (!(bibId001.charAt(6) == 'y' || bibId001.charAt(6) == 'u'
					|| bibId001.charAt(6) == 'v' || bibId001.charAt(6) == 'x')) { // base
																					// record
		...
		if (baseCnt == 0) {
			...
			setReject(status, outMarc,
					"Record Rejected - more than one bib record");
		}

		if (log.isInfoEnabled()) {
			log.info("Merge:" + dir.getPath() + " base:" + baseCnt + " - hold:"
					+ holdCnt + " - status:" + status.recordStatus);
		}
	...
	/*
	 * 6/3/2011 dbb Details of this step (constructing 009 and appending
	 * holdings) are specific to STF.
	 */

	private void appendHoldMarc(MarcRecord outMarc, MarcRecord inMarc) {
		inMarc.deleteFields("001", "840");
		inMarc.deleteFields("901", "999");
		// outMarc.deleteFields(list);
		appendAll(outMarc, inMarc);
	}
	...
	private Vector<MarcRecord> getRecords(File dir) {
		Vector<MarcRecord> arr = new Vector<MarcRecord>(100);
		...
		try {
			...
			throw new RuntimeException("Exception: for record " + dir.getPath()
					+ " error:" + ex.toString());
		}
		if (log.isDebugEnabled()) {
	 ...
	 * 
	 * @param dir
	 *            File entry to directory to be deleted
